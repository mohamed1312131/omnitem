...
(function () {
  ...
  if (form) {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (!ok) {
        showFormError("Veuillez compléter les champs requis.");
        return;
      }

      var formData = new FormData(form);
      if (!formData.get("cf-turnstile-response")) {
        showFormError("Veuillez valider la protection anti-spam.");
        return;
      }
      ...
      fetch("/api/contact", {
        ...
        .then(function (result) {
          ...
          showFormError(result.data && result.data.error ? result.data.error : "Une erreur est survenue. Merci de réessayer.");
          resetTurnstile();
        })
        .catch(function () {
    ...
    form.querySelectorAll("input,textarea").forEach(function (f) {
      f.addEventListener("input", function () { f.style.borderColor = ""; clearFormError(); });
    });
  }
